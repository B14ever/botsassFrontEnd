...
export default function UsagePage() {
  ...
  const getRemainingDays = (endString?: string) => {
    if (!endString) return t("resets_soon");
    const end = new Date(endString);
    const today = new Date();
    const diff = end.getTime() - today.getTime();
    const days = Math.ceil(diff / (1000 * 60 * 60 * 24));
    if (days <= 0) return t("resets_today");
    if (days === 1) return t("resets_in_one");
    return t("resets_in", { days });
  };
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      {isLoading ? (
        ...
        <div className="p-5 rounded-lg border border-destructive/20 bg-destructive/5 text-sm text-destructive">
          <p className="font-semibold">{t("unable_load")}</p>
          <p className="mt-1 text-xs opacity-80">{t("unable_load_sub")}</p>
        </div>
      ) : usage ? (
        <div className="space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 rounded-xl border border-border bg-card gap-3">
            <div>
              <p className="text-xs text-muted-foreground font-medium">{t("billing_period")}</p>
              <p className="text-sm font-bold text-foreground mt-0.5">
                {new Date(usage.period_start).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                <span className="mx-2 text-muted-foreground/40 font-light">{t("to")}</span>
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-foreground">{t("ai_bots")}</p>
                      <p className="text-[10px] text-muted-foreground">{t("active_agents")}</p>
                    </div>
                  </div>
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-foreground">{t("web_pages")}</p>
                      <p className="text-[10px] text-muted-foreground">{t("scraped_pages")}</p>
                    </div>
                  </div>
                  <span className="text-sm font-bold text-foreground">
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-foreground">{t("monthly_chats")}</p>
                      <p className="text-[10px] text-muted-foreground">{t("chat_messages")}</p>
                    </div>
                  </div>
                  <span className="text-sm font-bold text-foreground">
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-foreground">{t("pdf_pages")}</p>
                      <p className="text-[10px] text-muted-foreground">{t("processed_pages")}</p>
                    </div>
                  </div>
                  <span className="text-sm font-bold text-foreground">
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-foreground">{t("reports_gen")}</p>
                      <p className="text-[10px] text-muted-foreground">{t("generated_docs")}</p>
                    </div>
                  </div>
                  <span className="text-sm font-bold text-foreground">
